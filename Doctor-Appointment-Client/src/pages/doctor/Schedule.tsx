...
import { isFutureOrTodayDate, isPositiveNumber } from '../../utils/validation';
...
export function Schedule() {
  ...
  async function handleCreateSlot(e: React.FormEvent) {
    ...
    else if (!isFutureOrTodayDate(date)) newErrors.date = 'Please choose a date that is today or in the future.';
    if (!startTime) newErrors.startTime = 'Start time is required.';
    if (!endTime) newErrors.endTime = 'End time is required.';
    if (startTime && endTime && startTime >= endTime) newErrors.endTime = 'End time must be after start time.';
    if (!isPositiveNumber(maxPatients) || parseInt(maxPatients, 10) < 1) newErrors.maxPatients = 'Patients must be at least 1.';
    setErrors(newErrors);
    ...
    try {
      ...
      if (res.success) {
        ...
        setErrors({});
        loadSlots();
      } else setMessage(res.message || 'Failed to create slot');
      ...
      <form className={styles.form} onSubmit={handleCreateSlot}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 80px', gap: '0.75rem' }}>
          <div className={styles.formGroup}>
            ...
            <input id="date" className={styles.formInput} type="date" value={date} onChange={(e) => { setDate(e.target.value); if (errors.date) setErrors((x) => ({ ...x, date: '' })); }} min={new Date().toISOString().split('T')[0]} aria-invalid={!!errors.date} />
            <FieldError message={errors.date} />
          </div>
          <div className={styles.formGroup}>
            <label className={styles.formLabel}>Start</label>
            <input id="startTime" className={styles.formInput} type="time" value={startTime} onChange={(e) => { setStartTime(e.target.value); if (errors.startTime) setErrors((x) => ({ ...x, startTime: '' })); }} aria-invalid={!!errors.startTime} />
            <FieldError message={errors.startTime} />
          </div>
          <div className={styles.formGroup}>
            <label className={styles.formLabel}>End</label>
            <input id="endTime" className={styles.formInput} type="time" value={endTime} onChange={(e) => { setEndTime(e.target.value); if (errors.endTime) setErrors((x) => ({ ...x, endTime: '' })); }} aria-invalid={!!errors.endTime} />
            <FieldError message={errors.endTime} />
          </div>
          <div className={styles.formGroup}>
            <label className={styles.formLabel}>Patients</label>
            <input id="maxPatients" className={styles.formInput} type="number" min="1" value={maxPatients} onChange={(e) => { setMaxPatients(e.target.value); if (errors.maxPatients) setErrors((x) => ({ ...x, maxPatients: '' })); }} aria-invalid={!!errors.maxPatients} />
            <FieldError message={errors.maxPatients} />
          </div>
        </div>
